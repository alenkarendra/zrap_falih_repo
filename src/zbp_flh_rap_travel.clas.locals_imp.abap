...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD get_features.
    ...
    READ ENTITIES OF zflh_rap_travel IN LOCAL MODE
      ENTITY Travel
      ...
      IF travel-AgencyID IS INITIAL OR NOT line_exists( agencies_db[ agency_id = travel-AgencyID ] ).
        ...
        APPEND VALUE #( %tky    = travel-%tky
                    ...
                    %msg    = NEW zcm_rap_1(
                                  ...
                                  textid = zcm_rap_1=>agency_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id = travel-CustomerID ] ).
        ...
        APPEND VALUE #( %tky    = travel-%tky
                    ...
                    %msg    = NEW zcm_rap_1(
                                  severity = if_abap_behv_message=>severity-error
                                  textid = zcm_rap_1=>customer_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_1(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_1=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_1(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_1=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
      ...
      IF is_update_requested = abap_true.
        ...
        IF has_before_image = abap_true.
          ...
          IF update_granted = abap_false.
            APPEND VALUE #( %tky        = travel-%tky
                            %msg        = NEW zcm_rap_1( severity = if_abap_behv_message=>severity-error
                                                            textid   = zcm_rap_1=>unauthorized )
                          ) TO reported-travel.
          ENDIF.
...
*  ENDIF.
*
*  " Tambahkan error message jika update tidak diizinkan
*  IF update_granted = abap_false.
*    APPEND VALUE #( %tky        = travel-%tky
*                    %msg        = NEW zcm_rap_1( severity = if_abap_behv_message=>severity-error
*                                                textid   = zcm_rap_1=>unauthorized )
...
*ENDIF.
  ...
  METHOD is_update_granted.
    IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZFLHSTAT04'
        ID 'ZFLHSTAT04' FIELD travel_status
        ID 'ACTVT' FIELD '02'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZFLHSTAT04'
        ID 'ZFLHSTAT04' DUMMY
